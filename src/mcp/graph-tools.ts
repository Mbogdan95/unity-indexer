...
export function handleDetectCycles(
  store: Store,
  params: { edge_types?: string[]; max_length?: number; max_cycles?: number },
): object {
...
export function registerGraphTools(server: McpServer, resolveStore: StoreResolver): void {
  ...
  server.registerTool(
    ...
    {
      description:
        "Detect circular dependencies in the graph. Returns cycles up to max_cycles (default 200); response includes truncated:true when the cap is hit — increase max_cycles to see more.",
      inputSchema: {
        edge_types: z
          .array(z.string())
          .optional()
        ...
        max_cycles: z
          ...
          .optional()
          .describe(
            "Max number of cycles to return (default 200); response includes truncated:true if capped",
          ),
        project: z
          .string()
          .optional()
