...
function bound<T>(items: T[], limit: number | undefined, defaultLimit: number): T[] {
...
export function handleGetSceneHierarchy(
  ...
  const byParent = new Map<string | null, typeof allGOs>();
  ...
  if (filterLower !== undefined) {
    ...
    for (const go of allGOs) {
      const matches =
        go.name.toLowerCase().includes(filterLower) || go.tag.toLowerCase().includes(filterLower);
      if (!matches) continue;
      let cur: (typeof allGOs)[number] | undefined = go;
  ...
  const buildNode = (go: (typeof allGOs)[number], depth: number): HierarchyNode => {
    const children = (byParent.get(go.file_id_local) ?? []).filter(
      (c) => included === null || included.has(c.file_id_local),
    );
...
export function handleListAssets(store: Store, params: { type?: string; limit?: number }): object {
...
export function handleFindReferences(
  store: Store,
  params: { guid_or_name: string; depth?: number; limit?: number },
): object {
...
export function handleFindComponents(
  store: Store,
  params: { type: string; scene?: string; limit?: number },
): object {
...
export function registerTools(server: McpServer, resolveStore: StoreResolver): void {
  ...
  server.registerTool(
    ...
    {
      description:
        "Get the GameObject hierarchy (nested tree) for a scene or prefab file. " +
