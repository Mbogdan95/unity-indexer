...
describe('handleGetSceneHierarchy', () => {
  ...
  it('filters by name', () => {
    ...
    const roots = result.roots as Array<Record<string, unknown>>;
    expect(roots.every(r => String(r.name).toLowerCase().includes('player') || String(r.tag).toLowerCase().includes('player'))).toBe(true);
  });
...
describe('handleListScripts', () => {
  ...
  it('filters to zero when no monobehaviours match false + no non-MB scripts exist... is ok', () => {
    ...
    const nonMbResult = handleListScripts(store, { is_monobehaviour: false }) as Record<string, unknown>;
    const nonMbScripts = nonMbResult.scripts as Array<Record<string, unknown>>;
    // total = monobehaviour + non-monobehaviour
    const mbResult = handleListScripts(store, { is_monobehaviour: true }) as Record<string, unknown>;
...
describe('handleGetScriptDetail', () => {
  it('returns class_name=PlayerController with members having signature', () => {
    const result = handleGetScriptDetail(store, { class_name: 'PlayerController' }) as Record<string, unknown>;
  ...
  it('returns error for unknown class', () => {
    const result = handleGetScriptDetail(store, { class_name: 'NonExistent' }) as Record<string, unknown>;
    expect(result.error).toBeDefined();
  });
});
...
describe('handleSearch', () => {
  ...
  it('scoped search for scripts', () => {
    const result = handleSearch(store, { query: 'Player', scope: 'scripts' }) as Record<string, unknown>;
    const results = result.results as Array<Record<string, unknown>>;
...
describe('handleResolveGuid', () => {
  ...
  it('returns error for unknown GUID', () => {
    const result = handleResolveGuid(store, { guid: 'deadbeef00000000' }) as Record<string, unknown>;
    expect(result.error).toBeDefined();
  });
});
