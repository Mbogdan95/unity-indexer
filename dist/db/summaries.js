 ...
 */
export function generateComponentSummary(components, guidToClassName) {
    return components
        .map((c) => {
 ...
 */
export function generateSubtreeSummary(name, childNames) {
 ...
 */
export function generateFieldSummary(fields, guidNames) {
    const parts = [];
 ...
 */
export function generateApiSummary(script) {
    const lines = [];
    // Header line
    const bases = [];
    if (script.baseClass)
        bases.push(script.baseClass);
    bases.push(...script.interfaces);
    const header = bases.length > 0 ? `${script.className} : ${bases.join(", ")}` : script.className;
    lines.push(header);
    // Group members by kind
    const fields = script.members.filter((m) => m.kind === "field" && (m.attributes.includes("SerializeField") || m.access === "public"));
    ...
    if (fields.length > 0) {
        const fieldParts = fields.map((f) => {
            const attrs = f.attributes.length > 0 ? f.attributes.map((a) => `[${a}]`).join(" ") + " " : "";
 ...
 */
export function generateMemberSignature(member) {
    const attrPrefix = member.attributes.length > 0 ? member.attributes.map((a) => `[${a}]`).join(" ") + " " : "";
 ...
 */
export function generateFileSummaryLine(type, fileName, stats) {
    switch (type) {
        case "scene": {
