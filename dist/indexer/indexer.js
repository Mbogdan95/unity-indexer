...
export class Indexer {
    ...
    projectRoot;
    benchmark;
    guidToClassCache = null;
    pendingScenesAndPrefabs = [];
    constructor(store, projectRoot, benchmark) {
        this.store = store;
    ...
    async indexEssential() {
        ...
        const asmdefs = otherFiles.filter((f) => f.endsWith(".asmdef"));
        const assets = otherFiles.filter((f) => f.endsWith(".asset"));
        this.pendingScenesAndPrefabs = otherFiles.filter((f) => f.endsWith(".unity") || f.endsWith(".prefab"));
     ...
     */
    async indexScenesAndPrefabsBackground() {
    ...
    async indexBatchAsync(files, batchSize = 500) {
        for (let i = 0; i < files.length; i += batchSize) {
            await new Promise((resolve) => setImmediate(resolve));
            const batch = files.slice(i, i + batchSize);
            this.store.transaction(() => {
    ...
    // ---------------------------------------------------------------------------
    insertEdge(sourceType, sourceId, targetType, targetId, edgeType, sourceFileId, metadata = null) {
    ...
    // ---------------------------------------------------------------------------
    indexFileInternal(relativePath) {
        ...
        if ((fileType === "scene" || fileType === "prefab" || fileType === "asset") &&
    ...
    indexScene(fileId, relativePath, content, contentHash) {
        ...
        const scriptCount = parsed.gameObjects.reduce((sum, go) => sum + go.components.filter((c) => c.typeName === "MonoBehaviour").length, 0);
    ...
    indexPrefab(fileId, relativePath, content, contentHash) {
        ...
        const hasCustomScripts = parsed.gameObjects.some((go) => go.components.some((c) => c.typeName === "MonoBehaviour"));
        const importance = computeFileImportance({
            incomingRefCount: 0,
            outgoingRefCount: parsed.references.length,
            hasCustomScripts,
    ...
    indexAssetFile(fileId, relativePath, content, contentHash) {
    ...
    indexScript(fileId, relativePath, content, contentHash) {
     ...
     */
    indexScriptCrossEdges(relativePath) {
        const fullPath = join(this.projectRoot, relativePath);
        let content;
        try {
            content = readFileSync(fullPath, "utf8");
    ...
    indexAsmDef(fileId, relativePath, content, contentHash) {
        const parsed = parseAsmDef(content);
        const depSummary = parsed.references.length > 0 ? `refs: ${parsed.references.join(", ")}` : "no references";
    ...
    // ---------------------------------------------------------------------------
    storeGameObjects(fileId, gameObjects, guidToClass) {
        ...
        const insertRecursive = (go, depth) => {
            ...
            for (const child of children) {
                ...
                if (childSubtreeDepth > maxChildSubtreeDepth)
                    maxChildSubtreeDepth = childSubtreeDepth;
            }
    ...
    storeReferences(fileId, references) {
    ...
    updateProjectSummary() {
        ...
        const description = `Unity project with ${String(sceneCount)} scene${sceneCount !== 1 ? "s" : ""}, ` +
        ...
        for (const ref of topRefs) {
            ...
            if (file?.type === "meta") {
                ...
                if (assetFile?.type === "script") {
                    ...
                    if (script)
                        hotScripts.push(script.class_name);
                }
            }
        }
        ...
        for (const asm of assemblies) {
            assemblyStructure[asm.name] = JSON.parse(asm.references);
        }
