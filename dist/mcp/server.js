...
function uniqueName(base, existing) {
    if (!existing.has(base))
        return base;
    let i = 2;
    while (existing.has(`${base}-${String(i)}`))
        i++;
...
export async function startServer(rootDir) {
    ...
    const resolveStore = (projectName) => {
        if (projectName !== undefined && projectName !== "") {
            const p = projects.get(projectName);
            if (!p) {
                throw new Error(`Unknown project "${projectName}". Available: ${[...projects.keys()].join(", ")}`);
            }
            return p.store;
        }
        if (projects.size === 1) {
        ...
        throw new Error(`Multiple projects indexed. Specify "project" parameter. Available: ${[...projects.keys()].join(", ")}`);
    };
    ...
    for (const [name, project] of projects) {
        server.registerResource(`${name}-summary`, `unity://${name}/summary`, { description: `Project overview for ${name}. ~200 tokens.` }, () => ({
            contents: [
                {
        ...
        server.registerResource(`${name}-files`, `unity://${name}/files`, { description: `All files in ${name} sorted by importance. Paginated.` }, () => ({
            contents: [
                {
    ...
    server.registerResource("projects", "unity://projects", { description: "List all indexed Unity projects." }, () => ({
        contents: [
            {
                uri: "unity://projects",
                mimeType: "application/json",
                text: JSON.stringify([...projects.values()].map((p) => ({
                    name: p.name,
                    path: p.projectRoot,
    ...
    for (const { name, indexer, store } of toIndex) {
        try {
            ...
            log(`[${name}] scripts ready in ${elapsed}s — ${String(summary.script_count)} scripts, ${String(summary.scene_count)} scenes, ${String(summary.prefab_count)} prefabs`);
            ...
            void indexer.indexScenesAndPrefabsBackground().then(() => {
                ...
                log(`[${name}] background index complete — ${String(s.scene_count)} scenes, ${String(s.prefab_count)} prefabs`);
            });
        }
        catch (err) {
