...
import { analyzeFile, } from "../parsers/unused-analyzer.js";
...
export function handleGetSceneHierarchy(store, params) {
    ...
    if (!file)
        return { token_hint: 10, error: `File not found: ${params.scene}` };
...
export function handleGetPrefabStructure(store, params) {
...
export function handleListAssets(store, params) {
...
export function handleGetGameObject(store, params) {
    const file = store.getFileByPath(store.stripPrefix(params.scene));
    if (!file)
        return { token_hint: 10, error: `File not found: ${params.scene}` };
    ...
    if (!go)
        return { token_hint: 10, error: `GameObject not found: ${params.name_or_id}` };
    ...
    const response = {
        ...
        components: components.map((c) => ({
            ...
            serialized_fields: JSON.parse(c.serialized_fields),
        })),
    };
    return { token_hint: estimateTokens(response), ...response };
}
export function handleGetComponent(store, params) {
    const file = store.getFileByPath(store.stripPrefix(params.scene));
    if (!file)
        return { token_hint: 10, error: `File not found: ${params.scene}` };
    ...
    if (!go)
        return { token_hint: 10, error: `GameObject not found: ${params.game_object}` };
    const components = store.getComponentsByGameObject(go.id);
    const comp = components.find((c) => c.type_name.toLowerCase() === params.component_type.toLowerCase());
    if (!comp) {
        return {
            token_hint: 10,
    ...
    const response = {
        ...
        serialized_fields: JSON.parse(comp.serialized_fields),
    };
    return { token_hint: estimateTokens(response), ...response };
}
function resolveScriptNodeId(store, nodeId) {
...
export function handleGetScriptMember(store, params) {
    ...
    const params_parsed = JSON.parse(member.parameters);
    const attrs = JSON.parse(member.attributes);
    const response = {
        class_name: script.class_name,
        name: member.name,
        kind: member.kind,
...
export function handleFindReferences(store, params) {
...
export function handleFindDependencies(store, params) {
    if (params.depth !== undefined && params.depth > 1) {
        const nodeId = resolveToNodeId(store, params.guid_or_name);
        if (nodeId === null)
            return { token_hint: 10, error: `Cannot resolve: ${params.guid_or_name}` };
...
export function handleResolveGuid(store, params) {
...
export function handleSearch(store, params) {
...
export function handleFindComponents(store, params) {
    let fileId;
...
export function handleBatchGetScriptDetail(store, params) {
...
export function handleFindUnused(store, params) {
    // 1. Resolve scripts
    let scripts;
    if (params.class_name !== undefined) {
        ...
        if (!file)
            return { token_hint: 10, error: `File not found: ${params.file_path}` };
    ...
    for (const [fileId, fileScripts] of byFile) {
        ...
        const classes = fileScripts.map((script) => {
            const members = store.getScriptMembers(script.id);
        ...
        const hasFindings = result.unused_usings.length > 0 ||
            result.classes.some((c) => c.unused_fields.length > 0 || c.unused_locals.length > 0 || c.unused_methods.length > 0);
    ...
    const summary = {
        ...
        total_unused_fields: results.reduce((n, r) => n + r.classes.reduce((m, c) => m + c.unused_fields.length, 0), 0),
        total_unused_locals: results.reduce((n, r) => n + r.classes.reduce((m, c) => m + c.unused_locals.length, 0), 0),
        total_unused_methods: results.reduce((n, r) => n + r.classes.reduce((m, c) => m + c.unused_methods.length, 0), 0),
    };
...
// ---------------------------------------------------------------------------
export function registerTools(server, resolveStore) {
    const toContent = (obj) => ({
    ...
    server.registerTool("get_prefab_structure", {
    ...
    server.registerTool("get_game_object", {
    ...
    server.registerTool("get_script_detail", {
        description: "Get detailed info about a C# class: members with signatures and line numbers, relationships (callers/callees/implementors), and file_path. " +
    ...
    server.registerTool("batch_get_script_detail", {
        description: "Get full details for multiple scripts in one call. Accepts class names or script:N node IDs. " +
            "Reduces tool-call overhead when analysing related classes.",
        inputSchema: {
            class_names: z
                .array(z.string())
                .describe("Array of class names or script:N node IDs (e.g. ['PlayerController', 'HealthSystem'])"),
            project: z
                .string()
                .optional()
                .describe("Project name (required if multiple projects indexed)"),
    ...
    server.registerTool("get_script_member", {
    ...
    server.registerTool("find_unused", {
        description: "Find unused using directives, private fields, local variables, and non-public methods " +
